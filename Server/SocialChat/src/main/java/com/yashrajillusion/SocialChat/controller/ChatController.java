...
public class ChatController {
    ...
    @GetMapping("/chat/{id}")
    public ResponseEntity<Chat> getChatById(@PathVariable("id") String id) throws Exception {
    ...
    public ResponseEntity<List<Chat>> getAllChatsByUserId(@PathVariable("userId") String userId) throws Exception {
        return new ResponseEntity<>(chatService.findAllChatsForUser(UUID.fromString(userId)), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Chat> createChat(@RequestBody Chat chat) throws Exception {
        ...
        if(!chat.getIsGroupChat()){
            ...
            if(!chatId.isEmpty()) {
                ...
                return new ResponseEntity<>(chat_data, HttpStatus.OK);
            }
        }
        List<User> members = userService.findAllByUserIds(memberIds);
        chat.setMembers(members);
        Chat create_chat = chatService.createChat(chat);
        return new ResponseEntity<Chat>(create_chat, HttpStatus.CREATED);
    }

    @PutMapping("/chat/update/{id}")
    public ResponseEntity<Chat> renameChat(@PathVariable("id") String id, @RequestBody Chat chat) throws Exception {
        ...
        return new ResponseEntity<Chat>(chatService.createChat(chatData), HttpStatus.OK);
    }
}
